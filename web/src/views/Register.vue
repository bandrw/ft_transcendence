...
export default defineComponent({
	...
	methods: {
		async register() {
			...
			this.isLoading = true
			let r = await axios.get('http://localhost:3000/users/checkExist', {
			...
			const createRes: userCreate = await axios.post('http://localhost:3000/users/create', {
			...
			if (createRes.ok) {
				...
				const loginRes: userLogin = await axios.post('http://localhost:3000/users/login', {login: this.inputLogin})
					.then(res => {
						return res.data
					})
					.catch(err => this.errors = err)
